	...
	if(content_type === "bill") {
	  mongo.connect(mongoUri, function(err, db) {
			return db.collection('bills', function(er, c) {
				return c.find({id: voted_id}).toArray(function(err, bills) {
					mapped_bills = _.map(bills, function(item) {
							if(vote_type === "up"){
								return {
									_id: item._id,
									date: req.body.date,
									upvoters: _.uniq(item.upvoters.concat(user)),
									downvoters: _.reject(item.downvoters,function(elem) { elem === user }),
								...
								return {
									_id: item._id,
									date: req.body.date,
									upvoters: _.reject(item.upvoters,function(elem) { elem === user }),
	  ...
	  mongo.connect(mongoUri, function(err, db) {
			return db.collection('comments', function(er, c) {
				return c.find({id: voted_id}).toArray(function(err, bills) {
					mapped_comments = _.map(comments, function(item) {
							if(vote_type === "up"){
								return {
									_id: item._id,
									text: item.text,
